...
Y al final, ¿qué es un objeto? Un objeto se compone de atributos, que son básicamente características del objeto, así como de métodos, que son cosas que le pedimos que haga al objeto. En el ejemplo de la figura a continuación, vemos algunos atributos del objeto Perrito, como el nombre, la raza y el tamaño, así como algunos métodos como 'buscar' (=fetch) y 'hacerse el muerto' (=play_dead). La estructura básica de un objeto se da a continuación.
